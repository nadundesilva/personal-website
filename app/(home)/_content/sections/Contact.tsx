...
const Contact = (): React.ReactElement => (
    <div className="border-border bg-card relative mb-16 overflow-hidden rounded-xl border p-8 shadow-sm sm:p-12 md:mb-24 md:p-16">
        {/* Decorative blobs */}
        <div
            aria-hidden
                ...
                <div className="flex flex-wrap justify-center gap-3 lg:justify-start">
                    {Object.values(Profiles).map((profile) => (
                        <Link
                            ...
                            href={profile.link}
                            target="_blank"
                            className="border-primary/33 hover:border-primary text-foreground hover:text-primary hover:bg-primary/3 flex items-center gap-2 rounded-md border px-4 py-2 text-sm no-underline hover:no-underline motion-safe:transition-colors motion-safe:duration-150"
                        >
                            <profile.Icon size={15} aria-hidden />
                            {profile.name}
                        </Link>
                    ))}
                </div>
