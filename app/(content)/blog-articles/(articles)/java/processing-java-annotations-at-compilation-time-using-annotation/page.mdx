...
export const metadata = {
    title: "Processing Java Annotations at compile time using Annotation Processors",
    description:
...
Java annotations can be used for storing metadata that would have an impact on how the programs would be executed. Apart from the annotations that are available in Java, you can write your own [Custom Annotations](./java/unleashing-the-power-of-java-custom-annotations) as well. However, these annotations would not have any effect on their own.
...
```java title="AnnotationProcessorExample.java"
...
```plaintext title="META-INF/services/javax.annotation.processing.Processor"
com.foo.FooProcessor
