export const _copyright =
     ...
     *
     * © 2018 Nadun De Silva. All rights reserved.
     */
    undefined;
...
```java title="MyFirstAnnotation.java"
...
```java title="MyFirstAnnotatedClass.java"
...
```java title="TargetRestrictedAnnotation.java"
...
```java title="RetentionRestrictedAnnotation.java"
...
I have written [another post](./java/reading-java-annotations-on-the-fly) introducing the Reflections API which can be used for reading annotations at run-time when the `@Retention(RetentionPolicy.RUNTIME){:java}` annotation is used.
